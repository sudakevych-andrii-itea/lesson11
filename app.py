...
class ReviewBlock:
    ...
    def _get_current_message(self, chat_id):
        return self.MESSAGES[self._get_user_state(chat_id)]

    def _start_command(self, message):
        chat_id = message.chat.id
        self.bot.send_message(message.chat.id, 'Hello. Leave your review.')
        try:
            State.objects.create(chat_id=chat_id)
        except Exception:
            self._update_user_state(chat_id, 0)
        self.bot.send_message(chat_id, self._get_current_message(chat_id))
    ...
    def _review_command(self, message):
        ...
        self.review_dict[self.REVIEW_STEPS[state]] = message.text
        self._update_user_state(chat_id, state + 1)
        if self._get_user_state(chat_id) >= len(self.MESSAGES):
            Review.objects.create(**self.review_dict)
            self._update_user_state(chat_id, 0)
